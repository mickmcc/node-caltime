...
let timeSpan = function timeSpanCtor(inHours,
  ...
  if(((hours*MSECS_PER_HOUR)
    ...
    // console.log(`timeSpan() end time [msecs]: ${((hours*MSECS_PER_HOUR)+(minutes*MSECS_PER_MIN)+(seconds*1000)+(milliseconds)+(inDurationMins*MSECS_PER_MIN)+(inDurationSecs*1000)+(inDurationMSecs))}`); // debug
    // console.log(`timeSpan() limit [msecs]: ${MAX_MINS_PER_DAY*MSECS_PER_MIN}`); // debug
   ...
   */
  that.getTotalDuration = function getTotalDurationMSecsFunc() {

    let retval = 0;
    retval += (durationMins*MSECS_PER_MIN);
    retval += (durationSecs*1000);
    retval += durationMSecs;
    return retval;
  }
