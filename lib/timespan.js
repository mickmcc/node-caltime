  ...
  that.subtract = function subtractFunc(inTimeSpan) {
    ...
    if (that.isIntersect(inTimeSpan)
        ...
        && calcEndMs(that) >= calcEndMs(inTimeSpan)) {
          // time-spans start at the same time, only one remainder time-span.
          if (calcBeginMs(that) === calcBeginMs(inTimeSpan)
            ...
            retval.push(newSpan);
          // timespans end at the same time. only one remainder timespan.
          } else if (calcEndMs(that) === calcEndMs(inTimeSpan)
                    && calcBeginMs(that) < calcBeginMs(inTimeSpan)) {
            ...
            newSpan = timeSpan(newBeginSplit.hours,
                                ...
                                newDurationSplit.milliseconds);
            retval.push(newSpan);
          // two remainder timespans.
          // TODO refactor - can combine with previous clauses
