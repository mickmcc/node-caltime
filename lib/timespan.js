...
let timeSpan = function timeSpanCtor(inHours,
  ...
  that.subtract = function subtractFunc(inTimeSpan) {
    ...
    const newBegin = (calcBeginMs(that) < calcBeginMs(inTimeSpan))?calcBeginMs(that):calcBeginMs(inTimeSpan);
    const newEnd = (calcEndMs(that) > calcEndMs(inTimeSpan))?calcEndMs(that):calcEndMs(inTimeSpan);
    if(that.isIntersect(inTimeSpan)) {
      retval = timeSpan();
    }
