...
let timeRule = function timeRuleFunc(inTimeSpan, inConstraint, inDay, inTZ, inBegin=null, inEnd=null) {
  ...
  that.generateDateSpans = function generateDateSpansFunc(inBegin, inEnd) {
    ...
    searchBegin = (inBegin.getTime() < begin.getTime())?begin:inBegin;
    searchEnd = (inEnd.getTime() > end.getTime())?end:inEnd;
    ...
    while(beginCounter.toDate().getTime() < searchEnd.getTime()) {
      //console.log(`generateDateSpans() loop beginCounter: ${beginCounter.format()}`); // FIXME
  ...
  const generateDateSpan = function generateDateSpanFunc(inBegin, inEnd) {
    ...
    if((constraint ===  modconst.CONSTRAINT_DAY_OF_WEEK
      ...
      newDateSpan = makeDateSpan(inBegin);
              ...
              && constraint !==  modconst.CONSTRAINT_DAY_OF_MONTH)
            && beginMoment.date() === findDayOfMonth(constraint, day, beginMoment.year(), beginMoment.month())) {
      newDateSpan = makeDateSpan(inBegin);
    }
...
const makeDateSpan = function makeDateSpanFunc(inDate) {
  ...
  if(_.isDate(inDate) === false) {
    throw new Error('Invalid argument. Function expects a Date object as argument.');
  }
 ...
 * @throws {Error}
 */
const findDayOfMonth = function findDayOfMonthFunc(inConstraint, inDay, inYear, inMonth) {

  let retval = 0;
  let tempMoment = null;
  // used to find first, second etc. instance of a day during the month
  let multiplier = 0;
  //console.log(`findDayOfMonth() inConstraint: ${inConstraint}, inDay: ${inDay}, inYear: ${inYear}, inMonth: ${inMonth}`); // FIXME
  if(_.isInteger(inConstraint) === false
  ...
  if(inConstraint ==  modconst.CONSTRAINT_LAST_OF_MONTH) {
    ...
    if(tempMoment.year() === inYear
      ...
      throw new Error('Internal error calculating a rule with "last of month" constraint');
    }
  }
  else {
    tempMoment.day(inDay);
    ...
    if(tempMoment.year() === inYear
      ...
      throw new Error('Internal error calculating a rule with "*th of month" constraint');
    }
  }
  return retval;
}
