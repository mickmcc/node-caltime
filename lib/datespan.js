...
let dateSpan = function dateSpanFunc(inBegin,
   ...
   * @param {Object} inDateSpan Another instance of DateSpan. An Error is
  ...
  that.isEqual = function isEqualFunc(inDateSpan) {
    ...
    if (_.isObject(inDateSpan) === false) {
      throw new Error('Method expects a valid DateSpan object.');
    }
    if (that === inDateSpan
   ...
   * @throws Error
   * TODO returned array must always be sorted in ascending order.
   * TODO add argument to allow subtraction to be forced when there is insufficient overlap.
   */
