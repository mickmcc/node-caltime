 ...
 * is the total duration of the DateSpans.
 * @throws {TypeError}
 */
let calcDuration = function calcDurationFunc(inSpans, inDurationType) {
  let retval = 0;
  let newBegin = null;
  let newEnd = null;
  if (_.isArray(inSpans) === false) {
  ...
  const merged = mergeSpans(inSpans);

  for (let i=0; i<merged.length; i++) {
    let currentSpan = merged[i];
  }
  return retval;
};
