 ...
 * @throws {TypeError}
 */
let calcDuration = function calcDurationFunc(inSpans, inDurationType, inTZ=null) {
  let retval = 0;
  let timezone = inTZ;
  let mergedSpans = null;
  // beginning of first whole unit of time
  let fullSpanBegin = null;
  // end of last whole unit of time
  ...
  console.log(`calcDuration() inSpans.length: ${inSpans.length}, mergedSpans.length: ${mergedSpans.length}`); // debug
  ...
  for (let i=0; i<mergedSpans.length; i++) {
    ...
    else if(inDurationType === modconst.DURATION_NATURAL_SECS) {
      fullSpanBegin = moment(currentSpan.getBegin(), timezone);
      fullSpanBegin.endOf('second');
      fullSpanEnd = moment(currentSpan.getEnd(), timezone);
      fullSpanEnd.startOf('second');
      retval = fullSpanEnd.difference(fullSpanBegin, 'seconds');
      if(fullSpanBegin.difference(currentSpan.getBegin()) !== 0) {
        retval++;
      }
      if(fullSpanEnd.difference(currentSpan.getEnd()) !== 0) {
        retval++;
      }
    }
    else {
    ...
    console.log(`calcDuration() i: ${i}, retval: ${retval}`); // debug
  }
  return retval;
};
