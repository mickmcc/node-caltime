 ...
 * specified.  An error is thrown if both inEnd and inDurationMins are specified.
 * @param {Number} [inDurationSecs=0] Integer representing the [seconds] component
 * of the duration. Value defaults to zero if not specified.
 * An error is thrown if both inEnd and inDurationMins are specified.
 * @param {Number} [inDurationMSecs=0] Integer representing the [milliseconds]
...
let dateSpan = function dateSpanFunc(inBegin,
  ...
  let durationMSecs = inDurationMSecs;

  if(_.isNil(inBegin)
        || _.isDate(inBegin) === false) {
    throw new Error('Invalid inBegin argument passed to DateSpan constructor.');
  };
  if (_.isInteger(inDurationMins)
        && (inDurationMins < 0
            || inDurationMins > Number.MAX_SAFE_INTEGER)) {
    throw new Error('Invalid inDurationMins argument passed to DateSpan constructor.');
  };
  if (_.isInteger(inDurationSecs)
        && (inDurationSecs < 0
            || inDurationSecs > MAX_SECS)) {
    throw new Error('Invalid inDurationSecs argument passed to DateSpan constructor.');
  };
  if (_.isInteger(inDurationMSecs)
        && (inDurationMSecs < 0
            || inDurationMSecs > MAX_MSECS)) {
    throw new Error('Invalid inDurationMSecs argument passed to DateSpan constructor.');
  };
  if(_.isDate(inEnd)
      && (_.toSafeInteger(inDurationMins) > 0
          || _.toSafeInteger(inDurationSecs) > 0
          || _.toSafeInteger(inDurationMSecs)) > 0) {
    throw new Error('inEnd and (inDurationMins|inDurationSecs|inDurationMSecs) cannot be specified together.');
  }
  if(_.isDate(inEnd) === false
      && (_.isInteger(inDurationMins) === false
          || _.isInteger(inDurationSecs) === false
          || _.isInteger(inDurationMSecs) === false)) {
    throw new Error('inEnd or (inDurationMins|inDurationSecs|inDurationMSecs) must be specified.');
  }
  if(_.isDate(inEnd)
  ...
  if(_.isDate(inEnd)) {
    ...
    durationMins = delta / MSECS_PER_MIN;
    delta = delta - (durationMins * MSECS_PER_MIN);
  ...
  /**
   * Create a new DateSpan by merging two slices of time which overlap. If there
   * is no overlap then the method will return null.
   * @param {@link module:schedtime/datespan} inSpan Other DateSpan object which
   * will be merged with this datespan.
   * @return {@link module:schedtime/datespan|null} New DateSpan object or null if
   ...
   * @return {Array|null} Array containing one or two new DateSpan objects. These
   * are the remainders after the subtraction. null is returned if there wasn't
   * sufficient overlap of the date-spans to complete the subtraction.
   * @throws Error
   */
...
/**
 * Merge multiple, overlapping date-spans within an array.
