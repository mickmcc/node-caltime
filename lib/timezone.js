...
let timeZone = function timeZoneCtor(inTZ) {
  ...
  if(_.isString(inTZ) === false) {
    throw new Error('Invalid argument passed to TimeZone constructor.');
  }
   ...
   * previous instance of midnight. Function throws an error if the argument
   * is not a valid date.
   ...
   */
    ...
    if(_.isDate(inDate) === false) {
      throw new Error('Invalid date passed to function.');
    }
   ...
   * @param {object} inDate Date from which the function begins calculating the
   * next instance of midnight. Function throws an error if the argument
   * is not a valid date.
   * @return {object} Date which is after inDate.
   */
   ...
   * timezone.
   * @param {object} inDate Date which is to be checked. Function throws an
   ...
   * @param {object} inDate Date for which the function calculates the day
  ...
  that.dayOfWeek = function dayOfWeekFunc(inDate) {
    ...
    let tzMoment = null;
    if(_.isDate(inDate) === false) {
      throw new Error('Invalid argument passed to function');
    }
    tzMoment = moment.tz(inDate, tz);
    if(tzMoment.isValid() === false) {
      throw new Error('Failed to create moment object.');
